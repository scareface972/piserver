...
class Freebox(modules.Switch):
	...
	def execute(self, key, longPress=False):
		...
		if self.fbx_ok and is_power_key:
			self.state = self.fbx.getTvStatus()
			print("Tv is currently " + ("on" if self.state else "off"))
		...
		if '/' in key:
			...
			if k[1].isdigit():
				...
				canal = k[2]
				print("-> switch to canal after on:", canal)
...
class FreeboxOSCtrl:
	...
	def __init__(self, debug=False):
		...
		self.registrationSaveFile = "freebox.json"
	...
	def _stopAirplayReceiver(self, name):
