...
static PyObject *LibChaconError;
static PyObject * libchacon_send(PyObject *self, PyObject *args);

static PyMethodDef LibChaconMethods[] = {
	{"send",  libchacon_send, METH_VARARGS, "Send command to chacon interruptor."}
};

static struct PyModuleDef libchacon_module = {
   PyModuleDef_HEAD_INIT,
   "libchacon",   /* name of module */
...
PyMODINIT_FUNC
PyInit_libchacon(void) {
	PyObject *m;

	m = PyModule_Create(&libchacon_module);
	if (m == NULL)
		return NULL;

	LibChaconError = PyErr_NewException("libchacon.error", NULL, NULL);
	Py_INCREF(LibChaconError);
	PyModule_AddObject(m, "error", LibChaconError);
	return m;
}
