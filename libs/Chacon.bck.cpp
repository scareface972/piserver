...
void receive() {
	...
	for(;;) {
		...
		if (i>0) {
			if (sender > 0 && sender != emitter && (received[0] != sender || received[1] != recipient || received[2] != on)) {
				received[0] = sender;
...
static PyObject * Chacon_receive(PyObject *self, PyObject *args) {
	...
	for(;;) {
		...
		PyObject *arglist = Py_BuildValue("(i,i)", received[0], received[1]); //, received[2]
		PyObject *ret = PyEval_CallObject(callback, arglist);
		if (ret == NULL) log("PyEval_CallObject failed");
		else {
			PyArg_ParseTuple(ret, "s", &name);
			Py_DECREF(ret);
		}
		Py_DECREF(arglist);
